...
while getopts ":i:o:p:t:r:xdgvw:h" opt; do
    case $opt in
        i)
            INPUT=$OPTARG
            ;;
        ...
        #   Option r a été ajouter pour pouvoir inclure des fichiers de preconfig
        r)
            RESDIR=$OPTARG
            if [[ ! -e $RESDIR ]]; then
                echo "$RESDIR does not exist. Aborting."
                exit 4
            elif [[ ! -d $RESDIR ]]; then
                echo "$RESDIR is not a directory. Aborting."
                exit 4
            fi
            ;;
